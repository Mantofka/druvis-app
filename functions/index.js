...
exports.compressImage = functions
  ...
  .onFinalize(async (object) => {
    ...
    if (!filePath.includes("sections/")) {
      return console.log(
        `Can't handle uploaded image. Current image is uploaded to the wrong directory for minimizing.`
      );
    }
...
exports.compressVideo = functions.storage
  ...
  .onFinalize(async (object) => {
    ...
    if (!filePath.includes("sections/")) {
      return console.log(
        `Can't handle uploaded video. Current video is uploaded to the wrong directory for minimizing.`
      );
    }
